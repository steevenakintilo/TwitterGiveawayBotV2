...
class TwitterBot():
    ...
    def start(self) -> bool:
        ...
        print(old_password,new_password,current_username,new_username,new_bio,new_location,new_name,self.otp_acc,self.otp_code)
        ...
        if need_login:
            ...
            if check_login is False:
                if self.login(current_username,old_password) is False:
                    ...
                    print(f"Bad login on creation for https://x.com/{self.username}")
                    self.browser.close()
                ...
                if self.otp_acc:
                    if self.set_otp_code(self.otp_code) is False:
                        ...
                        print(f"Bad otp login on creation for https://x.com/{self.username}")
        ...
        if done:
            ...
            try:
                ...
                account_to_tag = f"{print_file_content("../../random_account_to_tag.txt").split("\n")[0]},{new_username}"
                print("Account to tag: " , account_to_tag)
                write_into_file("../../account_to_tag.txt",account_to_tag+"\n")
            except:
                traceback.print_exc()
                print("toto erreru")
            
        print("rorororotototo")
        self.del_first_line("../../random_ok_username.txt")
        self.del_first_line("../../random_bio.txt")
        self.del_first_line("../../random_city.txt")
        self.del_first_line("../../random_name.txt")
        self.del_first_line("../../random_account_to_tag.txt")
